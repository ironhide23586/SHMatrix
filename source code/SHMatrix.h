...
#define SHMATRIX_API __declspec(dllimport)
...
public:
  SHMATRIX_API SHMatrix(const cublasHandle_t &cublas_handle_arg,
  ...
  SHMATRIX_API void Equate(SHMatrix &src_shmatrix);
  SHMATRIX_API void Reallocate(std::vector<int> &dims, mem_location mem_loc = GPU,
                  bool copy_original = false, bool default_init = false,
                  float init_val = 0.0f);

  SHMATRIX_API void Print(bool print_elem = true);
  SHMATRIX_API void Move2GPU();
  SHMATRIX_API void Move2CPU();

  SHMATRIX_API void Clear();

  SHMATRIX_API void GaussianInit(float mean = 0.0f, float stddev = 0.1f);
  SHMATRIX_API void UniformInit(float lower = -0.5f, float higher = 0.5f);

  SHMATRIX_API static float GetGaussianNum(float mean, float stddev);
  SHMATRIX_API static float GetUniformNum(float lower, float higher);

  SHMATRIX_API void CommitUnaryOps(); //commits the transpose & scaling operations
  SHMATRIX_API void CommitTranspose(); // commits all lazy-tranpose ops
  SHMATRIX_API void CommitScale(); // commits all lazy-scale ops

  SHMATRIX_API SHMatrix& T(); //Transpose operation
  SHMATRIX_API SHMatrix& Scale(float scale_arg); //Scalar multiplication
  SHMATRIX_API static void Dot(cublasHandle_t cublas_handle, SHMatrix &A,
                  SHMatrix &B, SHMatrix &C); //Matrix Dot Product

  // Returns pointer to data at desired location (GPU or CPU)
  SHMATRIX_API static float* DataPointerAtLoc(SHMatrix& arg, mem_location desired_loc);

  SHMATRIX_API void operator*=(SHMatrix &arg);
  SHMATRIX_API void operator+=(SHMatrix &arg);
  SHMATRIX_API void operator-=(SHMatrix &arg);
  SHMATRIX_API void operator/=(SHMatrix &arg);

  SHMATRIX_API void operator*=(float arg);
  SHMATRIX_API void operator+=(float arg);
  SHMATRIX_API void operator-=(float arg);
  SHMATRIX_API void operator/=(float arg);
